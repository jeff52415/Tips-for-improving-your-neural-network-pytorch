...
"Modify from : https://github.com/RangiLyu/EfficientNet-Lite"
"reference : https://github.com/tensorflow/tpu/tree/master/models/official/efficientnet/lite"
    ...
    def __init__(
        ...
        if expand_ratio != 1:
            ...
            if group_normalization:
                if oup % 32 != 0:
                    self._bn0 = nn.GroupNorm(8, oup)
                else:
                    self._bn0 = nn.GroupNorm(32, oup)
            else:
        ...
        if group_normalization:
            if oup % 32 != 0:
                self._bn1 = nn.GroupNorm(8, oup)
            else:
                self._bn1 = nn.GroupNorm(32, oup)
        else:
        ...
        if group_normalization:
            if final_oup % 32 != 0:
                self._bn2 = nn.GroupNorm(8, final_oup)
            else:
                self._bn2 = nn.GroupNorm(32, final_oup)
        else:
...
class EfficientNetLite(nn.Module):
    def __init__(
        ...
        for i, stage_setting in enumerate(mb_block_settings):
            ...
            layers_count.append(num_repeat - 1)

        drop_possibility = torch.linspace(start=1, end=0.5, steps=sum(layers_count))
        drop_possibility_group = []
        n = 0
        for group, number in enumerate(layers_count):
            drop_possibility_group.append([])
            for _ in range(number):
                if stochastic_depth:
                    drop_possibility_group[group].append(drop_possibility[n])
                else:
                    drop_possibility_group[group].append(None)
                n += 1

        for i_stage_setting, drop_possibility in zip(
            enumerate(mb_block_settings), drop_possibility_group
        ):
            i, stage_setting = i_stage_setting
            stage = nn.ModuleList([])
            (
                num_repeat,
                kernal_size,
